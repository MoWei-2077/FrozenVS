...
public:
	...
	Server(Frozen& frozen, Settings& settings, ManagedApp& managedApp, SystemTools& systemTools,
		Doze& doze, Freezer& freezer) :
		frozen(frozen), settings(settings), managedApp(managedApp),
	...
	void serverThreadFunc() {
		...
		//const int addrLen = offsetof(sockaddr_un, sun_path) + 15; // addrLen大小是 首个占位符 '\0' 加 "frozenServer" 的字符长度
		//const sockaddr_un serv_addr{ AF_UNIX, "\0frozenServer" }; // 首位为空[0]=0，位于Linux抽象命名空间
		...
		while (true) {
			...
			while (true) {
				...
				if (recvLen == 1414873344 || recvLen == 4281684) {
					...
					frozen.logFmt("数据格式异常 recvLen[%u] HEX[%s]", recvLen,
	...
	void handleCmd(const MANAGER_CMD appCommand, const int recvLen, const int clnt_sock) {
		...
		case MANAGER_CMD::getAppCfg: {
			...
			for (const auto& [uid, info] : managedApp.getRaw()) {
				...
				ptr[intLen++] = info.isPermissive ? 1 : 0;
			}
		...
		case MANAGER_CMD::setAppCfg: {
			...
			for (int i = 0; i < intSize;) {
				...
				if (managedApp.contains(uid)) {
					if (managedApp.FREEZE_MODE_SET.contains(freezeMode))
						...
						frozen.logFmt("错误配置: UID:%d freezeMode:%d isTolerant:%d", uid,
							freezeMode, isPermissive);
				}
				else
					frozen.logFmt("特殊应用: UID:%d, 已强制自由后台", uid);
			}
            ...
            if (runningUids.size()) {
                ...
                frozen.logFmt("解冻策略变更的应用: %s", tips.c_str());
            };
		...
		case MANAGER_CMD::setAppLabel: {
			...
			for (const string& str : Utils::splitString(string(recvBuf.get(), recvLen),
				...
				if (managedApp.without(uid) || str.length() <= 6)
					frozen.logFmt("解析名称错误 [%s]", str.c_str());
