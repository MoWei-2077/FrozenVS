...
public:
	...
	SystemTools(Frozen& frozen, Settings& settings) :
	...
	void InitLMK() {
		...
		if (!access("/sys/module/lowmemorykiller/parameters", F_OK)) {
			...
			if (!Utils::writeString("/sys/module/lowmemorykiller/parameters/enable_lmk",
				"1", 2))
				frozen.log("调整lmk参数: 设置 enable_lmk 失败");
			if (!Utils::writeString("/sys/module/lowmemorykiller/parameters/minfree",
				minfree, sizeof(minfree)))
				frozen.log("调整lmk参数: 设置 minfree 失败");
		}
		if (frozen.moduleEnv == "Magisk") {
			string cmd;
		...
		else if (frozen.moduleEnv == "KernelSU") {
	...
	int readBatteryWatt() {
		...
		if (settings.enableCurrentFix) {
			if (abs(current) > 100'000) { // 如果单位是毫安，那么电流大于100A不现实，所以单位应该是微安，这种情况不能开启电流校准
				...
				frozen.log("电流校准不应开启, 已自动关闭");
				frozen.log(settings.save() ? "⚙️设置成功" : "🔧设置文件写入失败");
			}
		}
		else {
			current /= 1000; // uA -> mA
	...
	void checkBattery() {
		...
		if (lastCapacity == 0) { // 开机时
			...
			if (charge_full_design) {
				frozen.logFmt("🔋电池 设计容量: %dmAh", charge_full_design / 1000);
				int health = 100 * charge_full / charge_full_design;
				if (40 < health && health <= 100) {
					frozen.logFmt("🔋电池 当前容量: %dmAh", charge_full / 1000);
					frozen.logFmt("🔋电池 健康程度: %d%%", health);
				}
			}

			if (40 < battery_soh && battery_soh <= 100)
				frozen.logFmt("🔋电池 健康程度(内置): %d%%", battery_soh);

			if (cycle_count)
				frozen.logFmt("🔋电池 循环次数: %d", cycle_count);

			frozen.log("🔋电池 数据由系统提供, 仅供参考");
		}
		else {
			...
			frozen.logFmt("%s到 %d%%  %s%s了%d%%  %.2fw %.1f℃",
	...
	void getCPU_realtime(const uint32_t availableMiB) {
		...
		if (fp) {
			...
			while (true) {
				...
				if (coreIdx > 8) {
					frozen.logFmt("CPU可能超过8核, 暂不支持: coreIdx:%d", coreIdx);
					break;
				}
