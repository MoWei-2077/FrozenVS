...
public:
    ...
    ManagedApp(Freezeit& freezeit, Settings& settings) : freezeit(freezeit), settings(settings) {
        cfgPath = "/data/adb/modules/Frozen/appcfg.txt";
        labelPath = "/data/adb/modules/Frozen/applabel.txt";
    ...
    static constexpr int UID_START = 10000;
    static constexpr int UID_END = 14000;
    static constexpr int appMaxNum = UID_END - UID_START;
    ...
    void readCmdPackagesAll(map<int, string>& _allAppList) {
        ...
        static constexpr const char* cmdList[] = { "/system/bin/cmd", "cmd", "package", "list", "packages", "-U",
                                 nullptr };
    ...
    void readCmdPackagesThird(map<int, string>& _thirdAppList) {
        ...
        static constexpr const char* cmdList[] = { "/system/bin/cmd", "cmd", "package", "list", "packages", "-3",
                                 "-U", nullptr };
        VPOPEN::vpopen(cmdList[0], cmdList + 1, packageListBuff.get(), PACKAGE_LIST_BUF_SIZE);
        ss << packageListBuff.get();
        while (getline(ss, line)) {
    ...
    void updateIME2CfgTemp() {
        static constexpr const char* cmdList[] = { "/system/bin/ime", "ime", "list", "-s", nullptr };
        char buf[1024 * 4];
