...
public:
	...
	ManagedApp(Frozen& frozen, Settings& settings) : frozen(frozen), settings(settings) {
		cfgPath = frozen.modulePath + "/appcfg.txt";
		labelPath = frozen.modulePath + "/applabel.txt";
	...
	auto& getRaw() { return appInfoMap; }

	auto& operator[](const int uid) { return appInfoMap[uid]; }

	auto& operator[](const string& package) { return appInfoMap[uidIndex[package]]; }

	bool isBlackList(const int uid) { return contains(uid) && appInfoMap[uid].isBlacklist(); }

	size_t erase(const int uid) { return appInfoMap.erase(uid); }

	size_t size() { return appInfoMap.size(); }

	void clear() { appInfoMap.clear(); }

	bool contains(const int uid) { return appInfoMap.contains(uid); }

	bool contains(const string& package) { return uidIndex.contains(package); }

	bool without(const int uid) { return !appInfoMap.contains(uid); }

	bool without(const string& package) { return !uidIndex.contains(package); }

	auto& getLabel(const int uid) { return appInfoMap[uid].label; }
	...
	void updateHomePackage(const string& package) {
		...
		if (it == uidIndex.end()) {
			frozen.logFmt("当前桌面信息异常，建议反馈: [%s]", package.c_str());
			return;
		}
	...
	void updateAppList() {
		...
		if (allAppList.size() == 0) {
			frozen.log("没有应用或获取失败");
			return;
		}
		else {
			frozen.logFmt("刷新应用 %lu  系统[%lu] 三方[%lu]",
		...
		for (const auto& [uid, package] : allAppList) {
			...
			appInfoMap[uid] = appInfoStruct {
				.uid = uid,
				.freezeMode = isSYS ? FREEZE_MODE::WHITELIST : FREEZE_MODE::FREEZER, //freezeMode
				.isPermissive = true,    
				.delayCnt = 0,       
				.isSystemApp = isSYS,
                .startRunningTime = 0,
	...
	void loadConfigFile2CfgTemp() {
		...
		while (getline(file, line)) {
			if (line.length() <= 4) {
				frozen.logFmt("A配置错误: [%s]", line.c_str());
				continue;
			}

			auto value = Utils::splitString(line, " ");
			if (value.size() != 3 || value[0].empty()) {
				frozen.logFmt("B配置错误: [%s]", line.c_str());
				continue;
			}
			...
			if (!FREEZE_MODE_SET.contains(freezeMode)) {
				frozen.logFmt("C配置错误: [%s]", line.c_str());
				continue;
			}
	...
	void applyCfgTemp() {
		...
		for (const auto& [uid, cfg] : cfgTemp) {
			auto it = appInfoMap.find(uid);
			if (it == appInfoMap.end()) continue;

			it->second.freezeMode = cfg.freezeMode;
			it->second.isPermissive = cfg.isPermissive;
		}
		...
		if (homePackage.length() > 3) {
			...
			if (it != uidIndex.end())
				appInfoMap[it->second].freezeMode = FREEZE_MODE::WHITEFORCE;
		}
	}

	void saveConfig() {
		stackString<1024 * 64> tmp;
		for (const auto& [uid, cfg] : appInfoMap)
			if (cfg.freezeMode < FREEZE_MODE::WHITEFORCE)
				tmp.appendFmt("%s %d %d\n",
					cfg.package.c_str(),
					static_cast<int>(cfg.freezeMode),
					cfg.isPermissive ? 1 : 0);

		if (Utils::writeString(cfgPath.c_str(), *tmp, tmp.length))
			frozen.log("保存配置成功");
		else
			frozen.logFmt("保存配置文件失败: [%s]", cfgPath.c_str());
	}
	...
	void update2xposedByLocalSocket() {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (recvLen != 4) {
				frozen.logFmt("%s() 更新到Xposed 第%d次异常 sendLen[%lu] recvLen[%d] %d:%s",
			...
			case REPLY::SUCCESS:
				return;
			case REPLY::FAILURE:
				frozen.log("更新到Xposed失败");
				return;
			default:
				frozen.logFmt("更新到Xposed 未知回应[%d]", buff[0]);
				return;
			}
		}
		frozen.logFmt("%s() 工作异常, 请确认LSPosed中冻它勾选系统框架, 然后重启 sendLen[%lu]", __FUNCTION__,
			tmp.length());
	}
	...
	void loadLabelFile() {
		...
		if (!file.is_open()) {
			frozen.logFmt("读取应用名称文件失败: [%s]", labelPath.c_str());
			return;
		}
		...
		while (getline(file, line)) {
			if (line.length() <= 2) {
				frozen.logFmt("读取到错误包名: [%s]", line.c_str());
				continue;
			}
			...
			if (isalpha(line[0])) {// package####label
				auto value = Utils::splitString(line, "####");
				if (value.size() != 2) {
					frozen.logFmt("分割错误: [%s]", line.c_str());
					continue;
				}
				auto it = uidIndex.find(value[0]);
				if (it != uidIndex.end())
					appInfoMap[it->second].label = value[1];
			}
			else if (isdigit(line[0])) {  // uid label
				int uid = atoi(line.c_str());

				auto it = appInfoMap.find(uid);
				if (it != appInfoMap.end() && line.length() > 6)
	...
	void saveLabel() {
		...
		if (fd < 0) {
			frozen.logFmt("保存应用名称文件失败: [%s]", labelPath.c_str());
			return;
		}

		string tmp;
		tmp.reserve(1024L * 16);
		for (const auto& [uid, info] : appInfoMap)
