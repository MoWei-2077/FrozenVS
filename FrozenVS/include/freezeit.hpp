...
public:
    ...
    const char* modulePath = "/data/adb/modules/Frozen/";
    string moduleEnv{ "Unknown" };
    const char* workMode = nullptr;
    ...
    Freezeit(int argc) {
        ...
        toFileFlag = argc > 1; // 只要传输参数 就能实现本地日志 
        if (toFileFlag) {
        ...
        propPath = "/data/adb/modules/Frozen/module.prop";
    ...
    void setWorkMode(const char* mode) {
        workMode = mode;
    }
    ...
    size_t formatProp(char* ptr, const size_t maxSize, const int cpuCluster) {
        return snprintf(ptr, maxSize, "%s\n%s\n%s\n%s\n%s\n%d\n%s\n%s\n%s\n%s\n%u",
            ...
            cpuCluster, moduleEnv.c_str(), workMode,
            androidVerStr.c_str(), kernelVerStr.c_str(), extMemorySize);
    }

    int formatTime() {
        constexpr time_t time_offset = 8 * 3600L;
        const time_t timeStamp = time(nullptr) + time_offset;
