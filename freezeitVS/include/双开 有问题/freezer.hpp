...
private:
    ...
    // /sys/fs/cgroup/app/uid_%d/pid_%d/cgroup.freeze
    static constexpr const char* cgroupV2SystemUidPidPath = "/sys/fs/cgroup/system/uid_0/cgroup.freeze";


    static constexpr const char v2wchan[16] = "do_freezer_trap";      // FreezerV2冻结状态
    static constexpr const char v1wchan[16] = "__refrigerator";       // FreezerV1冻结状态
    static constexpr const char SIGSTOPwchan[16] = "do_signal_stop";  // SIGSTOP冻结状态
    static constexpr const char v2xwchan[16] = "get_signal";          // FreezerV2冻结状态 内联状态
    static constexpr const char pStopwchan[16] = "ptrace_stop";       // ptrace冻结状态
    static constexpr const char epoll_wait1_wchan[16] = "SyS_epoll_wait";
    static constexpr const char epoll_wait2_wchan[16] = "do_epoll_wait";
    static constexpr const char binder_wchan[32] = "binder_ioctl_write_read";
    static constexpr const char pipe_wchan[16] = "pipe_wait";
...
public:
    ...
    int getMainUid(int uid) {
        if (uid >= 99900 && uid <= 99999) {
            return uid - 99900;   // 按规则恢复主 UID
        }
        if (uid >= 99900000) {
            return uid % 100000;  // 多开 999xx 加 uid
        }
        return uid;
    }

    void getPids(const int UID, appInfoStruct& appInfo) {
        START_TIME_COUNT;
        
        const int mainUid = getMainUid(UID);
        appInfo.pids.clear();

        DIR* dir = opendir("/proc");
        if (dir == nullptr) {
            char errTips[256];
            Utils::FastSnprintf(errTips, sizeof(errTips), "错误: %s() [%d]:[%s]", __FUNCTION__, errno,
                strerror(errno));
        ...
        while ((file = readdir(dir)) != nullptr) {
            ...
            struct stat statBuf;
            if (stat(fullPath, &statBuf) && statBuf.st_uid != (uid_t)UID)continue;
            
            memcpy(fullPath + len + 6, "/cmdline", 8);
            fullPath[len + 14] = '\0';
            
            char readBuff[256];
            if (Utils::readString(fullPath, readBuff, sizeof(readBuff)) == 0)continue;
            const string& package = managedApp[mainUid].package;
            ...
            if (endChar != ':' && endChar != 0 && endChar != '_')continue;
    ...
    int handleProcess(appInfoStruct& appInfo, const bool freeze) {
        ...
        const int currentUid = appInfo.uid;
        if (freeze) {
            getPids(currentUid, appInfo);
        }
        else {
            erase_if(appInfo.pids, [currentUid](const int pid) {
                char path[32] = {};
                ...
                return (uid_t)currentUid != statBuf.st_uid;
                });
        }
        ...
        case FREEZE_MODE::FREEZER_BREAK: {
            if (workMode != WORK_MODE::GLOBAL_SIGSTOP) {
                static constexpr bool fuck = false;
                if (fuck) {
        ...
        if (settings.isWakeupEnable()) {
            ...
            if (freeze && appInfo.pids.size() && appInfo.isSignalOrFreezer()) {
                ...
                unfrozenTimeline[nextIdx] = currentUid;
            }
            else {
        ...
        if (freeze && appInfo.needBreakNetwork()) {
            const auto ret = systemTools.breakNetworkByLocalSocket(currentUid);
    ...
    void bootFreeze() {
        ...
        if (naughtyApp.size() == 0) {
            ...
            while ((file = readdir(dir)) != nullptr) {
                ...
                auto& appInfo = managedApp[mainUid];
    ...
    void printProcState() {
        ...
        int totalMiB = 0;
        // 关键修复 1: uidSet 存储的是实际运行的应用的 UID（包括多开 UID），用于准确统计
        std::set<int> uidSet; 
        std::set<int> pidSet;

        std::lock_guard<std::mutex> lock(naughtyMutex);
        naughtyApp.clear();
        ...
        while ((file = readdir(dir)) != nullptr) {
            ...
            const char* pidStr = file->d_name;
            const size_t pidLen = strlen(pidStr);
            const char* procPrefix = "/proc/";
            const size_t procPrefixLen = 6; // strlen("/proc/")

            // 路径缓冲区，用于构建 /proc/[pid]/...
            char procPath[64];
            
            // 检查路径长度是否溢出，以确保安全
            if (procPrefixLen + pidLen + 10 >= sizeof(procPath)) continue; 

            // 1. 构建基础路径 /proc/[pid]
            memcpy(procPath, procPrefix, procPrefixLen);
            memcpy(procPath + procPrefixLen, pidStr, pidLen);
            procPath[procPrefixLen + pidLen] = '\0'; // 关键：空终止符
            size_t currentLen = procPrefixLen + pidLen;
            
            // 2. 获取 UID (读取 /proc/[pid]/status)
            // 修复点 2: 修正路径构建，并读取 status 文件获取 UID
            memcpy(procPath + currentLen, "/status", 8); // "/status" + \0
            
            char statusContent[512] = {};
            if (Utils::readString(procPath, statusContent, sizeof(statusContent)) == 0) continue;
            
            const char* uidLine = strstr(statusContent, "Uid:\t");
            if (uidLine == nullptr) continue;
            
            int uid;
            if (sscanf(uidLine, "Uid:\t%d", &uid) != 1) continue; // 实际 UID

            // 检查 UID 是否在我们管理的范围内，并跳过前台和待处理应用
            if (!managedApp.contains(uid)) continue; 
            if (pendingHandleList.contains(uid) || curForegroundApp.contains(uid)) continue;

            auto& appInfo = managedApp[uid];
            if (appInfo.isWhitelist()) continue;

            // 3. 检查 cmdline (获取包名)
            // 修复点 3: 修正路径构建，指向 /proc/[pid]/cmdline
            memcpy(procPath + currentLen, "/cmdline", 9); // "/cmdline" + \0
            
            char readBuff[256]; // now is cmdline Content
            if (Utils::readString(procPath, readBuff, sizeof(readBuff)) == 0) continue;
            
            // 获取主 UID 来查找包名
            int mainUid = getMainUid(uid);
            const auto& package = managedApp[mainUid].package; // 使用主 UID 找到包名
            ...
            // 进程通过所有检查
            uidSet.insert(uid); // 插入实际 UID (包括多开 UID)
            pidSet.insert(pid);

            // 4. 获取内存 (读取 /proc/[pid]/statm)
            // 修复点 4: 修正路径构建，指向 /proc/[pid]/statm
            memcpy(procPath + currentLen, "/statm", 7); // "/statm" + \0
            
            Utils::readString(procPath, readBuff, sizeof(readBuff)); // now is statm content
            const char* ptr = strchr(readBuff, ' ');

            // Unit: 1 page(4KiB) convert to MiB.
            const int memMiB = ptr ? (atoi(ptr + 1) >> 8) : 0;
            totalMiB += memMiB;
            string finalAppLabel;

            const bool isClone = (mainUid != uid); 

            if (isClone) {
                // 查找主应用的 label，必须使用 mainUid 查找
                const std::string& mainLabel = managedApp.contains(mainUid)
                    ? managedApp[mainUid].label 
                    : appInfo.label; // 容错：使用克隆应用自己的 label (包名)

                finalAppLabel = "多开:" + mainLabel;
            } else {
                // 主应用
                finalAppLabel = appInfo.label;
            }
            
            stackString<256> procLabel(finalAppLabel.c_str(), finalAppLabel.length());

            // 如果是子进程，将子进程名附加到应用名称后
            if (readBuff[package.length()] == ':')
                procLabel.append(readBuff + package.length());


            if (appInfo.isAudioPlaying) {
                stateStr.appendFmt("%5d %4d 🎵正在播放 %s\n", pid, memMiB, procLabel.c_str());
                continue;
            }

            // 6. 获取 wchan (读取 /proc/[pid]/wchan)
            // 修复点 6: 修正路径构建，指向 /proc/[pid]/wchan
            memcpy(procPath + currentLen, "/wchan", 7); // "/wchan" + \0
            
            if (Utils::readString(procPath, readBuff, sizeof(readBuff)) == 0) {
                // 如果读取失败，进程可能已退出，跳过它。
                continue; 
            }
            ...
            if (!strcmp(readBuff, v2wchan) || !strcmp(readBuff, v2xwchan)) {
                ...
                stateStr.appendFmt("⚠️运行中(%s) %s\n", (const char*)readBuff, procLabel.c_str());
                naughtyApp.insert(uid); // 实际 UID
            }
        }
        closedir(dir);
        ...
        if (uidSet.empty()) { 
            ...
            // 总结: uidSet 存储实际 UID，能够正确统计主应用和多开应用的数量
            stateStr.appendFmt("\n总计 %d 应用 %d 进程, 占用内存 ", (int)uidSet.size(), (int)pidSet.size());
            stateStr.appendFmt("%.2f GiB", totalMiB / 1024.0);

            freezeit.log(std::string_view(stateStr.c_str(), stateStr.length));
        }
        stackString<64> tips;
    ...
    void processPendingApp() {
        ...
        while (it != pendingHandleList.end()) {
            ...
            if (appInfo.isAudioPlaying) {
                remainSec = 6; 
                freezeit.debugFmt("%s:%d 正在播放音频🎵, 延迟%d秒冻结", appInfo.label.c_str(), uid, remainSec);
                it++;
                continue;
            }
    ...
    void getAudioByLocalSocket() {
        unordered_set<int> currAudioApp;
        ...
        while (true) {
            ...
            if (static_cast<REPLY>(buff[uidCount]) != REPLY::SUCCESS) {
                freezeit.log("获取音频播放事件失败");
                return;
            }
            for (int oldUid : lastAudioApp)
                managedApp[oldUid].isAudioPlaying  = false;
            ...
            for (int i = 0; i < uidCount; ++i) {
                ...
                currAudioApp.insert(uid); 
                managedApp[uid].isAudioPlaying = true;   
            }
            
            lastAudioApp.swap(currAudioApp); 

        /*
            这里也可以遍历全部 managedApp
            查找所有 isAudioPlaying == true 的应用
            如果不在 currAudioApp 中 则说明音频已经停止播放
            需要将 isAudioPlaying 设为 false
            以便在下一次冻结时进行冻结
            但是性能开销高
        */
            END_TIME_COUNT;
            Utils::sleep_ms(1500);
            // 这里需要将UID添加到 audioApp 中 
            // 避免音频应用被误判为后台应用后被冻结 
            // 如果音频暂停播放 就需要冻结 如果音频继续播放 就需要给他6秒的不冻结的时间 
            // 如果6秒过后音频依旧播放 就继续赋予6秒的不冻结的时间
        }
    }
    ...
    void handlePendingIntent() {
        ...
        while (true) {
            ...
            for (int i = 0; i < uidCount; i++) {
                ...
                auto& appInfo = managedApp[uid];
                if (managedApp.contains(uid) && appInfo.isFreeze && !pendingHandleList.contains(uid))  {
    ...
    void getVisibleAppByShell() {
        ...
        while (getline(ss, line)) {
            ...
            int userId = 0;
            auto userPos = line.find("userId=");
            if (userPos != string::npos)
                userId = atoi(line.c_str() + userPos + 7);

            int uid = managedApp.getUid(package, userId);
            if (uid < 0) continue;
    ...
    void getVisibleAppByLocalSocket() {
        ...
        for (int i = 1; i <= UidLen; i++) {
            int& uid = buff[i];
            int mainUid = getMainUid(uid);
            if (managedApp.contains(mainUid)) curForegroundApp.insert(mainUid);
    ...
    int binderEventTriggerTask(void) {
        ...
        while (true) {
            ...
            bool NetWorkType = strncmp(u_info.msg, "type=Binder", 11);
            ...
            bool BinderType = strncmp(u_info.msg, "bindertype=free_buffer_full", 27);

            #if DEBUG_DURATION
                freezeit.logFmt("ReKernel发送的通知:%s", u_info.msg);
            #endif

            if (ptr != nullptr && !NetWorkType && oneway != 1 && !BinderType) {     

                const int uid = atoi(ptr + 7);
                auto& appInfo = managedApp[uid];
                //appInfo.isFrozen
                if (managedApp.contains(uid) && appInfo.isPermissive && appInfo.isFreeze && !pendingHandleList.contains(uid)) {
                    freezeit.logFmt("[%s] 接收到Re:Kernel的Binder信息, 类别: %s 类型: %s, 将进行临时解冻", managedApp[uid].label.c_str(), oneway ? "ASYNC" : "SYNC", NetWorkType ? "网络解冻" : "临时解冻");     
                    unFreezerTemporary(uid, 3);      
                }              
            }     
        }
    ...
    int NkBinderMagiskFunc(void) {
        ...
        while (true) {
            ...
            if (ptr != nullptr) {
                ...
                auto& appInfo = managedApp[uid];
                if (managedApp.contains(uid) && appInfo.isPermissive && !pendingHandleList.contains(uid) && appInfo.isBlacklist()) {
    ...
    void getBlackListUidRunning(set<int>& uids) {
        ...
        while ((file = readdir(dir)) != nullptr) {
            ...
            const int uid = statBuf.st_uid;
            int mainUid = getMainUid(uid);
            if (!managedApp.contains(uid) || managedApp[mainUid].isWhitelist())
                continue;
            ...
            char readBuff[256];
            if (Utils::readString(fullPath, readBuff, sizeof(readBuff)) == 0)continue;
            const auto& package = managedApp[mainUid].package;
            if (strncmp(readBuff, package.c_str(), package.length())) continue;
            const char endChar = readBuff[package.length()]; // 特例 com.android.chrome_zygote 无法binder冻结
            if (endChar != ':' && endChar != 0 && endChar != '_')continue;
