...
public:
    ...
    bool isCloneUid(int uid) const {
        return uid >= ManagedApp::CLONE_UID_START && uid < ManagedApp::CLONE_UID_END;
    }

    int getMainUidFromClone(int cloneUid) const {
        return cloneUid - 99900000;
    }
    ...
    void handleCmd(const MANAGER_CMD appCommand, const int recvLen, const int clnt_sock) {
        ...
        case MANAGER_CMD::getAppCfg: {
            ...
            for (const auto& [uid, appInfo] : managedApp.appInfoMap) {
                ptr[intCnt++] = uid;
        ...
        case MANAGER_CMD::setAppCfg: {
            ...
            for (const auto& [uid, appInfo] : managedApp.appInfoMap) {
                if (appInfo.freezeMode == FREEZE_MODE::WHITEFORCE ||
                    !newCfg.contains(uid) || appInfo.freezeMode == newCfg[uid].freezeMode)
                    continue;
        ...
        case MANAGER_CMD::setAppLabel: {
            ...
            string recv(recvBuf.get(), recvLen);
            for (const string& str : Utils::splitString(recv, "\n")) {
                ...
                string label = str.substr(pos);
                bool isClone = isCloneUid(uid);

                if (isClone) {
                    int mainUid = getMainUidFromClone(uid);
                    if (!managedApp.contains(mainUid)) {
                        freezeit.logFmt("未知多开UID [%s]", str.c_str());
                        continue;
                    }
                } else {
                    if (!managedApp.contains(uid)) {
                        freezeit.logFmt("未知UID [%s]", str.c_str());
                        continue;
                    }
                }

                if (isClone) label = "[多开]" + label;
