...
private:
    ...
    string homePackage;
    map<string, vector<int>> packageUidIndex;
    map<int, cfgStruct> cfgTemp;

    bool isSupportedUid(int uid) const {
        return (UID_START <= uid && uid < UID_END) ||
            (CLONE_UID_START <= uid && uid < CLONE_UID_END);
    }
...
public:
    ...
    const static int CLONE_UID_START = 99910000;
    const static int CLONE_UID_END = 99914000; 
    map<int, appInfoStruct> appInfoMap;

    appInfoStruct& operator[](const int uid) { return appInfoMap.at(uid); }
    const appInfoStruct& operator[](const int uid) const { return appInfoMap.at(uid); }

    void clear() { 
        appInfoMap.clear();
        packageUidIndex.clear();
    }

    bool contains(const int uid) const { return appInfoMap.contains(uid); }

    bool contains(const string& package) const {
        auto it = packageUidIndex.find(package);
        return it != packageUidIndex.end() && !it->second.empty();
    }

    bool isBlackList(const int uid) { return contains(uid) && appInfoMap.at(uid).isBlacklist(); }

    auto& getLabel(const int uid) { return appInfoMap.at(uid).label; }

    int getUid(const string& package, int userId = -1) const {
        auto it = packageUidIndex.find(package);
        if (it == packageUidIndex.end() || it->second.empty())
            return -1;
    ...
    int getUidOrDefault(const string& package, const int defaultValue) const {
        auto uid = getUid(package);
        return uid >= 0 ? uid : defaultValue;
    }
    ...
    void updateHomePackage(const string& package) {
        homePackage = package;
        const auto it = packageUidIndex.find(package);
        if (it == packageUidIndex.end() || it->second.empty()) {
            freezeit.logFmt("当前桌面信息异常，建议反馈: [%s]", package.c_str());
            return;
        }

        const int uid = it->second.front();
        if (contains(uid))
            appInfoMap[uid].freezeMode = FREEZE_MODE::WHITEFORCE;
    }

    void handleUidList(const string& uidPart, const string& packageName, const string& line, string_view sysEnd, map<int, string>& all, map<int, string>* third = nullptr) {
        size_t pos = uidPart.find(',');

        int uidMain  = atoi(uidPart.c_str());
        int uidClone = (pos != string::npos) ? atoi(uidPart.c_str() + pos + 1) : -1;

        auto add = [&](int uid) {
            if (!isSupportedUid(uid)) return;

            all[uid] = packageName;

            if (third) {
                if (!line.ends_with(sysEnd))
                    (*third)[uid] = packageName;
            }
        };
    ...
    bool readPackagesListA12(map<int, string>& _allAppList, map<int, string>& _thirdAppList) {
        ...
        while (getline(ss, line)) {
            ...
            char package[256], uidStr[64];
            if (sscanf(line.c_str(), "%255s %63s", package, uidStr) != 2)
                continue;

            string packageName(package);
            string uidPart(uidStr);

            handleUidList(uidPart, packageName, line, sysEnd, _allAppList, &_thirdAppList);
        }
    ...
    bool readPackagesListA10_11(map<int, string>& _allAppList) {
        ...
        while (getline(ss, line)) {
            ...
            if (!isSupportedUid(uid)) continue;
    ...
    void readCmdPackagesAll(map<int, string>& _allAppList) {
        ...
        const char* cmdList[] = { "/system/bin/cmd", "cmd", "package", "list", "packages", "-U", nullptr };
        ...
        while (getline(ss, line)) {
            ...
            if (idx == string::npos || idx < 10) continue;

            string packageName = line.substr(8, idx - 8);
            string uidPart = line.substr(idx + 5);

            handleUidList(uidPart, packageName, line, string_view{}, _allAppList, nullptr);
        }
    ...
    void readCmdPackagesThird(map<int, string>& _thirdAppList) {
        ...
        while (getline(ss, line)) {
            ...
            handleUidList(uidPart, packageName, line, string_view{}, _thirdAppList, nullptr);
        }
        END_TIME_COUNT;
    }
    ...
    void updateAppList() {
        ...
        for (const auto& [uid, package] : allAppList) {
            packageUidIndex[package].emplace_back(uid);        // 更新 按包名取UID
            const bool isSYS = !thirdAppList.contains(uid);
            if (contains(uid)) {
                auto& info = appInfoMap[uid];
                info.package = package;
                info.isSystemApp = isSYS;
                continue;
            }

            appInfoMap[uid] = appInfoStruct{
                .uid = uid,
        ...
        for (auto it = appInfoMap.begin(); it != appInfoMap.end();) {
            if (allAppList.contains(it->first)) {
                ++it;
                continue;
            }

            it = appInfoMap.erase(it);
        }
        END_TIME_COUNT;
    }
    ...
    void loadConfigFile2CfgTemp() {
        ...
        while (getline(file, line)) {
            ...
            if (isdigit(value[0][0])) {
                ...
                auto it = packageUidIndex.find(value[0]);
                if (it == packageUidIndex.end() || it->second.empty())continue;
                uid = it->second.front();
            }
    ...
    void updateIME2CfgTemp() {
        ...
        while (getline(ss, line)) {
            ...
            for (const int uid : it->second)
                cfgTemp[uid] = { FREEZE_MODE::WHITEFORCE, 0 };
        }
    }
    ...
    void applyCfgTemp() {
        for (auto& [uid, appInfo] : appInfoMap) {
            (void)uid;
        ...
        if (homePackage.length() > 3) {
            auto it = packageUidIndex.find(homePackage);
            if (it != packageUidIndex.end()) {
                for (const int uid : it->second)
                    if (contains(uid))
                        appInfoMap[uid].freezeMode = FREEZE_MODE::WHITEFORCE;
            }
    ...
    void saveConfig() {
        ...
        for (const auto& [uid, appInfo] : appInfoMap) {
            (void)uid;
    ...
    void update2xposedByLocalSocket() {
        ...
        for (const auto& [uid, appInfo] : appInfoMap) {
            ...
            if (appInfo.isPermissive)
                permissiveUids.emplace_back(uid);
        }
        tmp += '\n';
    ...
    void loadLabelFile() {
        ...
        while (getline(file, line)) {
            ...
            auto it = packageUidIndex.find(line.substr(0, splitIdx));
            if (it != packageUidIndex.end()) {
                for (const int uid : it->second)
                    if (contains(uid))
                        appInfoMap[uid].label = line.substr(splitIdx + 4);
            }
        }
        file.close();
    }
    ...
    void saveLabel() {
        ...
        tmp.reserve(1024L * 16);
        for (const auto& [uid, appInfo] : appInfoMap)
            if (uid > 0 && appInfo.package != appInfo.label) {
                tmp += appInfo.package;
                tmp += "####";
                tmp += appInfo.label;
                tmp += '\n';
